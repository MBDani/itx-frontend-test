...
export const ProductDetailsPage = () => {
  ...
  const isAddDisabled = selectedColor === null || selectedStorage === null || isAdding;
  ...
  if (isError || !product) {
    return (
      <div className="flex-1 flex flex-col justify-center items-center gap-4 text-center">
        ...
        <p className="text-gray-500">The mobile phone you are looking for doesn't exist or there was a network error.</p>
  ...
  const formattedCameras = [
    product.primaryCamera ? `Main: ${formatArrayField(product.primaryCamera)}` : null,
    product.secondaryCmera ? `Front: ${formatArrayField(product.secondaryCmera)}` : null // API typo secondaryCmera
  ].filter(Boolean).join(' | ') || '-';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 md:gap-8 lg:gap-12">
          ...
          <div className="p-8 md:p-12 lg:pr-16 flex flex-col">
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6 text-sm mb-8">
              <li className="flex flex-col"><span className="text-gray-500">CPU</span> <span className="font-medium text-gray-900">{product.cpu || '-'}</span></li>
              <li className="flex flex-col"><span className="text-gray-500">RAM</span> <span className="font-medium text-gray-900">{product.ram || '-'}</span></li>
              <li className="flex flex-col"><span className="text-gray-500">OS</span> <span className="font-medium text-gray-900">{product.os || '-'}</span></li>
              <li className="flex flex-col"><span className="text-gray-500">Resolution</span> <span className="font-medium text-gray-900">{product.displayResolution || '-'}</span></li>
              <li className="flex flex-col"><span className="text-gray-500">Battery</span> <span className="font-medium text-gray-900">{product.battery || '-'}</span></li>
              <li className="flex flex-col"><span className="text-gray-500">Cameras</span> <span className="font-medium text-gray-900 text-xs mt-0.5">{formattedCameras}</span></li>
              <li className="flex flex-col"><span className="text-gray-500">Dimensions</span> <span className="font-medium text-gray-900 text-xs mt-0.5">{product.dimentions || '-'}</span></li>
              <li className="flex flex-col"><span className="text-gray-500">Weight</span> <span className="font-medium text-gray-900">{product.weight ? `${product.weight}g` : '-'}</span></li>
            </ul>
            ...
            <div className="mt-auto space-y-6 bg-gray-50 p-6 rounded-2xl border border-gray-100">
              ...
              {/* Storage Selector */}
              <div>
                <span className="block text-sm font-medium text-gray-700 mb-3">Storage Capacity</span>
                <div className="flex flex-wrap gap-3">
                  {product.options.storages.map((storage) => {
                    ...
                    return (
                      ...
                      >
                        {storage.name}
                        {isSelected && <Check className="absolute -top-2 -right-2 h-4 w-4 bg-blue-600 text-white rounded-full p-0.5" />}
                      </button>
                    )
                  })}
                </div>
              ...
              {/* Color Selector */}
              <div>
                <span className="block text-sm font-medium text-gray-700 mb-3">Color</span>
                <div className="flex flex-wrap gap-3">
